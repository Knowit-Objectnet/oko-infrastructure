...
resource "aws_sns_topic_subscription" "subscriptions" {
    ...
    protocol = "sqs"
    endpoint = data.aws_sqs_queue.calendar.arn
}

resource "aws_sqs_queue_policy" "test" {
    queue_url = data.aws_sqs_queue.calendar.url

    policy = templatefile("policies/queue_policy.json.tmpl", {
        queue_arn = data.aws_sqs_queue.calendar.arn
        topic_arns = jsonencode([for topic in data.aws_sns_topic.topics: topic.arn])
        policy_id = "${data.aws_sqs_queue.calendar.name}-policy"
    })
}
